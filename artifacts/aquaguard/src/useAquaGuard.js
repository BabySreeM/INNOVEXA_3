...
const baseEvents = [
  { timestamp: Date.now() - 52000, type: 'SYSTEM', message: 'AquaGuard connected to local reserve network.' },
...
export function useAquaGuard() {
    ...
    const message = target === 'BRANCH_A'
      ...
      : target === 'BRANCH_B'
        ...
        : target === 'SOURCE_CRITICAL'
          ? 'Source critical threshold reached. Priority reserve protection engaged for Tank A.'
  ...
  const updateCommand = useCallback((key, value) => {
    ...
    const message = key === 'priority_tank'
      ...
      : key === 'manual_valve_override'
        ...
        : `${key} set to ${value}.`;
    addEvent('COMMAND', message);
  }, [addEvent, demoActive]);

  const resetControls = useCallback(() => {
    setData((current) => ({
      ...current,
      commands: { ...current.commands, estop_triggered: false, bypass_confirm: false },
      system: { ...current.system, last_updated: Date.now() },
    }));
    if (firebaseDatabase && !demoActive) {
    ...
    addEvent('COMMAND', 'Command flags reset. Resume remains available as a deliberate recovery action.');
  }, [addEvent, demoActive]);

  const emergencyStop = useCallback(() => {
    setData((current) => ({ ...current, system: { ...current.system, pump_on: false, last_updated: Date.now() }, commands: { ...current.commands, estop_triggered: true } }));
    if (firebaseDatabase && !demoActive) {
      set(ref(firebaseDatabase, 'commands/estop_triggered'), true).catch(() => undefined);
    }
    addEvent('EMERGENCY', 'Emergency Stop engaged. Pump halted; recovery requires an explicit resume.');
  }, [addEvent, demoActive]);

  const resume = useCallback(() => {
    setData((current) => ({ ...current, system: { ...current.system, pump_on: true, last_updated: Date.now() }, commands: { ...current.commands, estop_triggered: false } }));
    if (firebaseDatabase && !demoActive) {
      set(ref(firebaseDatabase, 'commands/estop_triggered'), false).catch(() => undefined);
    }
    addEvent('COMMAND', 'System resumed. Pump returned to commanded operation.');
  }, [addEvent, demoActive]);
  ...
  return {
    ...
    isConnected: demoActive || liveConnected,
