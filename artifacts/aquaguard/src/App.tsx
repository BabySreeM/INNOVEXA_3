...
function Controls({ data, connected, simulateLeakA, simulateLeakB, simulateSourceCritical, resetScenario, updateCommand, resetControls, emergencyStop, resume }: any) {
  ...
  return <div className="space-y-5">
    ...
    {disabled && <div className="flex items-center gap-2 rounded-xl border border-[#e4d29d] bg-[#f5ecd4] px-4 py-3 text-xs text-[#765e24]" data-testid="status-controls-disabled"><CircleHelp size={15} />Controls are disabled while disconnected. Run Demo Mode or connect Firebase to send commands.</div>}
  </div>;
}
